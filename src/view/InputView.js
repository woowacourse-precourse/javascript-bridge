 ...
 */
function validateBridgeSize(input, callback) {
  return () => {
 ...
 * @return {function(): void} 에러핸들러에게 전달할 콜백함수
 */
function validateCommand(input, allowedSet, callback) {
  return () => {
 ...
 */
function errorHandler(callback, onError) {
  try {
    callback();
  } catch (err) {
    Console.print(`\n${err.message}`);
...
const InputView = {
  ...
  readBridgeSize(callback) {
    ...
    Console.readLine('', (input) => errorHandler(
  ...
  readMoving(callback) {
    ...
    Console.readLine('', (input) => errorHandler(
      validateCommand(input, [BRIDGE.ABOVE, BRIDGE.BELOW], callback),
  ...
  readGameCommand(callback) {
    ...
    Console.readLine('', (input) => errorHandler(
      validateCommand(input, [GAME_COMMAND.RETRY, GAME_COMMAND.QUIT], callback),
