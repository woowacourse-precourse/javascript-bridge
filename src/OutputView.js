...
const OutputView = {
  ...
  printMapHelper(userSpace,bridgeArray){
    if(userSpace==='D'&&bridgeArray[0]==='D'){
      Output.bridgePrinterAbove.push('empty')
      Output.bridgePrinterBelow.push('O')
      return 'O'
    }
    if(userSpace==='U'&&bridgeArray[0]==='U') {
      Output.bridgePrinterAbove.push('O')
      Output.bridgePrinterBelow.push('empty')
      return 'O'
    }
    if(userSpace==="D"&&bridgeArray[0]==='U'){
      Output.bridgePrinterAbove.push('empty')
      Output.bridgePrinterBelow.push('X')
      return 'X'
    }    
    if(userSpace==='U'&&bridgeArray[0]==='D') {
      Output.bridgePrinterAbove.push('X')
      Output.bridgePrinterBelow.push('empty')
      return 'X'
    }
  },
