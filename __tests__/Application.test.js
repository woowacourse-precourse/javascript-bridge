...
describe('올바른 문자열 파악 함수 테스트', () => {
  ...
  test('메소드 이름은 "checkContain"로 정의된다.', () => {
    const METHOD_NAME = 'checkContain';

    expect(Application.checkContain.name).toEqual(METHOD_NAME);
  });

  test('대문자 U, 대문자 D를 입력을 제외한 값은 예외를 발생한다.', () => {
    expect(() => {
      const checkContain = Application.checkContain(INCLUDE_TARGET);
      const EXPECTED = 'K';

      checkContain(EXPECTED);
    }).toThrow(RANGE_ERROR_TEXT);
  });
  ...
  test('배열을 전달하지 않으면 예외를 발생시킨다.', () => {
    expect(() => {
      ...
      Application.checkContain(EXPECTED);
    }).toThrow(VALIDATION_ARRAY_TEXT);
  });
});
