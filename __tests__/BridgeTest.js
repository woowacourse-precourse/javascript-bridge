...
describe("브릿지 다리 생성 테스트", () => {
    test("정수를 입력하면 해당 길이 만큼의 다리가 생성 됩니다" ,()=>{
        ...
        expect(woowaBridge.bridge.getOriginalBridge().length).not.toBeGreaterThanOrEqual(6)
        expect(woowaBridge.bridge.getOriginalBridge().length).not.toBeLessThanOrEqual(2)
    })
