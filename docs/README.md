...
- [x] 예외는 throw 문을 사용해 발생, "[ERROR] ..." + 바로 종료가 아니라 그 부분부터 입력을 다시 받는다
- [x] ApplicationTest PASS 여부
- [x] 요구사항에서 명시하지 않는 한 파일, 패키지 이름 수정, 이동 금지
- [x] else 지양
- [x] InputView 에서만 입력받기- readLind()
- [x] BridgeGame 클래스에서 InputView, OutputView 사용 금지
...
- [x] 이동 판단 - move
  - [x] 위 배열, 아래 배열 분리
  - [x] \*\* 여기에 시도횟수 카운트 변수 같이 넣기
  - [x] 다리 : 0(D), 1(U)
  - [x] 경우의 수
    - [x] 입력: U /다리: 0 -> 위: O && 카운트 ++
    - [x] 입력: U /다리: 1 -> 위: X -> RQ 선택
    - [x] 입력: D /다리: 0 -> 아래: X -> RQ 선택
    - [x] 입력: D /다리: 1 -> 아래: O && 카운트 ++
  - [x] 입력이 U,D가 아니거나 다리가 0,1이 아니면 에러처리
  - [ ] O,X 결과가 틀리면 에러 처리 v
...
- [x] 이동 표시 - printMap v
  ...
  - [x] 일단 매치하고, 배열에 넣기 v
  - [x] 이동 판단에서 위: O일 경우 표시 + 아래에 공백 표시 v
  - [x] 이동 판단에서 위: X일 경우 표시 -> RQ v
  - [x] 이동 판단에서 아래: X일 경우 표시 -> RQ v
  - [x] 이동 판단에서 아래: O일 경우 표시 + 위에 공백 표시 v
  ...
  - [x] Application test로 체크 v

###

- [x] 건너다 실패

  - [x] 플레이어 입력 : 재시작(R) or 종료(Q)
  - [x] 에러 : R이나 Q입력이 아닐 경우 에러처리

  - [x] 재시작 로직 : 아까 "그 다리"로 실행 + 시도횟수 리셋
  - [x] 종료 로직 v
    - [x] 최종 게임 결과 (앞서 만든 결과 저장했다가 출력)
          다리 모양 출력,
