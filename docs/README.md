...
- [O] 선택한 칸을 이동할 수 있는지 없는지 판별한다.
  - [O] 다리 건너기에 성공했을 경우 다시 이동할 칸을 입력받는다.
  - [O] 다리 건너기에 실패했을 경우 재시작여부를 입력받는다.
    - [O] R을 입력했을 경우 이미 생성한 다리로 다시 시작한다.
    - [O] Q를 입력했을 경우 최종 게임 결과를 출력한다.
  - [O] OutputView 클래스의 printMap메서드를 이용해 현재까지의 다리 상태를 출력한다.
  - [O] `예외처리` 재시작 여부를 입력받을 때, R나 Q가 아닌 다른 문자를 입력할 경우 throw문을 이용해 에러문구를 출력하며 다시 입력받을 수 있도록 한다.
- [O] 다리 건너기 게임을 종료했을 경우 성공 여부 및 시도 횟수 출력
