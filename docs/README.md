...
- [X] 예외가 있다면 throw문을 통한 예외 발생, 에러 메세지 출력, 재입력을 실시한다.
  - [X] 입력 받은 다리 길이가 범위를 벗어난 경우
  - [X] 입력 받은 다리 길이가 숫자가 아닌 경우
  - [X] 입력 받은 이동 칸이 'U'나 'D'가 아닌 경우
  - [X] 입력 받은 재시작 여부가 'R'이나 'Q'가 아닌 경우
