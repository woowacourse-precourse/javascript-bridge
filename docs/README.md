...
4. [ ] 무작위 값이 `0`인 경우 D(아래 칸), `1`인 경우 U(위 칸)이 건널 수 있는 칸이된다.
...
2. [ ] 잘못된 값을 입력한 경우, `throw`로 [ERROR]"로 시작하는 에러 메시지를 발생시킨 후 다시 입력을 받는다.
...
2. [ ] 잘못된 값을 입력한 경우 `throw`로 [ERROR]"로 시작하는 에러 메시지를 발생시킨 후 입력을 다시 받는다.
