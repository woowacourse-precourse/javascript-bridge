...
- [x] 자동으로 생성할 다리 길이를 입력 받을 수 있다.
  - [x] 다리의 길이는 숫자만 입력받을 수 있다.
  - [x] 숫자의 범위는 3 이상 20 이하이다.
  - [x] 숫자는 0으로 시작할 수 없다.
  - [x] `예외 사항`이 있다면 예외를 발생시키고 에러 메세지를 출력 후 입력을 다시 받는다.
- [x] 다리를 생성할 때 위 칸과 아래 칸 중 건널 수 있는 칸은 0과 1 중 무작위 값을 이용하여 정할 수 있다.
  - [x] 무작위 값이 0인 경우 아래 칸, 1인 경우 위 칸이 건널 수 있는 칸이 된다.
  - [x] 위 칸을 건널 수 있는 경우 U, 아래 칸을 건널 수 있는 경우 D값으로 나타낸다.

### 2. 다리 건너기

- [x] 플레이어는 이동할 칸을 선택할 수 있다.
  - [x] 위 칸은 대문자 U, 아래 칸은 대문자 D를 입력하여 이동할 수 있다.
  - [x] 대문자 U, 대문자 D 이외의 값은 입력할 수 없다.
  - [x] `예외 사항`이 있다면 예외를 발생시키고 에러 메세지를 출력 후 입력을 다시 받는다.
...
- [x] 다리를 끝까지 건너면 게임이 종료된다.
  - [x] 게임이 종료되면 게임 성공 여부와 총 시도한 횟수가 출력된다.
- [x] 다리를 건너다 실패한 후 Q(종료)를 입력하면 게임이 종료된다.
  - [x] `예외 사항`이 있다면 예외를 발생시키고 에러 메세지를 출력 후 입력을 다시 받는다.
...
- [x] 다리 길이 입력값 유효성 검사
  - [x] 숫자가 아니면 예외처리한다.
  - [x] 0으로 시작하는 숫자를 입력했을 시 예외처리한다.
  - [x] 3~20 사이의 숫자가 아닌 경우 예외처리한다.
- [x] 이동할 칸 입력값 유효성 검사
  - [x] U(위)나 D(아래)가 아닐 경우 예외처리한다.
- [x] 재시작/종료 입력값 유효성 검사
  - [x] R(재시작)이나 Q(종료)가 아닐 경우 예외처리한다.

### 다리 생성 검사

- [x] 다리 생성 시, 0~1 중 하나가 출력되는지 검사
- [x] 다리 생성 시, 0, 1값에 맞게 다리 배열에 U와 D가 잘 들어가는지 검사

### 다리 이동 검사

- [x] 다리 이동 시, up배열과 down배열에 잘 들어가는지 검사
- [x] 다리 이동 시, 이동에 실패했는지 검사
