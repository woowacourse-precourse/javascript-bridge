...
다리 건너기 게임은 위아래 두 칸으로 이루어져 있고, 왼쪽에서 오른쪽으로 건너야한다. 위아래 둘 중 하나의 칸만 선택해 건널 수 있는 다리를 끝까지 건너가는 게임이다.
...
- [x] 라운드마다 플레이어가 이동할 칸을 입력 받는다.
  - 이동할 때 위 칸은 대문자 U, 아래 칸은 대문자 D 중 하나의 문자를 입력 받을 수 있다.
...
- [x] 이동할 때마다 생성된 다리 칸과 플레이어가 이동할 칸을 비교한다.
...
#### 5.1 게임 재시작 (다리 건너기 실패)

- [x] 게임 실패 시 재시작/종료 여부를 입력 받는다.
  - `R(재시작)`과 `Q(종료)` 중 하나의 문자를 입력 받는다.
  - R(재시작)일 때 처음 만든 다리를 재사용한다.
  - 재시작 할 때마다 총 시도 횟수가 늘어난다.
...
- [x] 다리 길이에 `3부터 20사이의 숫자` 이외의 값이 들어온 경우
- [x] 이동할 칸에 `'U'`와 `'D'` 이외의 값이 들어온 경우
- [x] 재시작/종료에 `'R'`와 `'Q'` 이외의 값이 들어온 경우
