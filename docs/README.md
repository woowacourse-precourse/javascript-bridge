...
2. 다리 길이 입력하는 기능 만들기
   ...
   - [ ] 출력과 입력: 다리 길이 입력 문구 보여주고 사용자가 다리 길이 입력
   - [ ] 입력값 유효성 검사: 다리길이는 3이상 20이하의 숫자
   - [ ] 예외: 3이상 20이하의 숫자이외의 모든 문자열은 예외 대상이다.
         올바른 값이 아니라면 아닐시, 유효성 검사에 실패할 시 "[ERROR]"로 시작하는 에러 메서지 출려하고 입력을 다시 받는다.

3. 다리를 건널 수 있는지에 대한 기능 만들기

   - [ ] 랜덤값 생성: 다리길이만큼 0과 1의 랜덤 숫자를 추출한다.
   - [ ] 매치: 랜덤 값과 이동방향을 매치시킨다.

4. 사용자가 선택한 칸이 이동할 칸인지 확인하는 기능 만들기

   - [ ] 출력과 입력: 사용자에게 이동할 칸 선택하라는 문구 출력하고 사용자가 이동할 칸 입력
   - [ ] 유효성 검사: 아래로 건널 수 있는 경우는 U, 위로 건널 수 있는 경우는 D로 나타낸다.(대문자)
